...
class Philanthropy extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            helpOut: [
                ...
                {title: 'Donate', shortDescription:'Donations Help Create a Sustainable Future', buttonText:'Donate Now', url: 'https://www.we-connect-the-dots.org/donate', description: "We seek strategic and enthusiastic partnerships that will drive the fiscal and operational growth of our organization. Your donations provide the resource to create systemic change in empowering young and old to build sustainable careers for themselves and their families.  Be a part of the change by making a one time donation or invest in reoccurring donations through a monthly or annual donation. Donations of all amounts are appreciated and recognized on our social media."},
    ...
    createProgramCard = () => {
        return(
            this.state.programs.map((program, key) => (
                <Col key={key} style={{ backgroundImage: `url('${program.image}')` }} className="PhilanthropyUrlCard">
                    <h1>{program.title}</h1>
    ...
    helpOutCard = () => {
        return(
            this.state.helpOut.map((program, key) => (
                <Row key={key} className="staticCard">
                    ...
                    <Col xs={8} className="cardColumns offWhite">
                        <Card style={{padding: "10px"}}>
                            ...
                            {typeof program.description === "string" ? <p>{program.description}</p> : <div>{program.description}</div>}
                            <p>{program.test}</p>
                        </Card>
                    </Col>
                </Row>
...
render() {
        return (
            <div className="philanthropyContainer">
                ...
                <br/>
                <div className="helpOutCardContainer">
                    { this.helpOutCard() }
                </div>
            </div>
        )
